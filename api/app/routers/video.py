...
def create_videos(channelname, folder, output_path):
    ...
    # Iterate through all webp files in the current directory
    img_files = sorted(glob.glob(folder+'*.webp'))
...
def create_shorts(channelname, folder, output_path):
    ...
    # Create a list of all images in the specified folder with supported extensions
    img_files = []
...
def get_image_orientation(folder_path):
    ...
    image_files = [file for file in files if file.endswith('.jpg') or file.endswith('.webp')]
